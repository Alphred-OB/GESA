    ...
    <div x-data="{ loading: true }" x-init="setTimeout(() => { loading = false }, 600)" class="mx-auto w-full max-w-6xl px-5 py-12 sm:px-6 lg:px-8">
        ...
        <div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="space-y-10">
            <header class="flex flex-col gap-4 rounded-3xl border border-[#16136a]/15 bg-white/85 p-6 shadow-lg shadow-[#16136a]/10 md:flex-row md:items-center md:justify-between">
                ...
                <div>
                    <a href="{{ route('student.dues.index') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
                        ...
                        <div class="p-8">
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    ...
                                    <button @click="selectedNetwork = 'mtn'" 
                                        ...
                                        <div x-show="selectedNetwork === 'mtn'" class="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-yellow-400 border-2 border-white flex items-center justify-center">
                                            <i class="ri-check-line text-yellow-900 text-xs font-bold"></i>
                                        </div>
                                    </button>
                            ...
                            <div class="mt-8 relative">
                                ...
                                <div x-show="selectedNetwork === 'mtn'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex gap-4 group">
                                            <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-yellow-400 text-[10px] font-black text-yellow-900 group-hover:scale-110 transition-transform">1</span>
                                            <p class="text-sm font-semibold text-slate-700 leading-normal">Dial <span class="text-yellow-600 font-bold">*170#</span></p>
                                        </div>
                                        <div class="flex gap-4 group">
                                ...
                                <div x-show="selectedNetwork === 'telecel'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-cloak class="space-y-6">
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex gap-4 group">
                                            <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-red-600 text-[10px] font-black text-white group-hover:scale-110 transition-transform">1</span>
                                            <p class="text-sm font-semibold text-slate-700 leading-normal">Dial <span class="text-red-600 font-bold">*110#</span></p>
                                        </div>
                                        <div class="flex gap-4 group">
                                ...
                                <div x-show="selectedNetwork === 'at'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-cloak class="space-y-6">
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex gap-4 group">
                                            <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-blue-600 text-[10px] font-black text-white group-hover:scale-110 transition-transform">1</span>
                                            <p class="text-sm font-semibold text-slate-700 leading-normal">Dial <span class="text-blue-600 font-bold">*110#</span></p>
                                        </div>
                                        <div class="flex gap-4 group">
                            ...
                            <div class="rounded-[2.3rem] bg-gradient-to-br from-[#1c198a] to-[#14125a] px-8 py-8">
                                <div class="space-y-8">
                                    <div class="flex items-center justify-between">
                                        <div>
                                            <p class="text-[11px] font-black uppercase tracking-[0.3em] text-white/40">Receiver</p>
                                    ...
                                    <div class="relative group cursor-pointer" @click="copyNumber()">
                                        ...
                                        <div class="mt-2 flex items-center justify-between">
                                            <span class="text-3xl font-mono font-black tracking-wider text-yellow-400 transition group-hover:text-yellow-300 group-hover:scale-[1.02] inline-block">{{ $settings['merchant_number'] }}</span>
                                        ...
                                        <p x-show="copySuccess" x-transition class="mt-2 text-[10px] font-bold uppercase tracking-widest text-emerald-400">Number Copied to clipboard!</p>
                                    </div>

                                    <div class="flex items-center gap-4 pt-4 border-t border-white/10">
                                        <div class="flex -space-x-2">
                                            <div class="h-8 w-8 rounded-full border-2 border-[#16136a] bg-yellow-400 flex items-center justify-center text-[10px] font-black">M</div>
                                            <div class="h-8 w-8 rounded-full border-2 border-[#16136a] bg-red-600 flex items-center justify-center text-[10px] font-black text-white">T</div>
                                            <div class="h-8 w-8 rounded-full border-2 border-[#16136a] bg-blue-600 flex items-center justify-center text-[10px] font-black text-white">A</div>
                                        </div>
                                        <p class="text-[10px] font-bold text-white/40 uppercase tracking-widest">Multi-Network Support</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <section class="overflow-hidden rounded-[3rem] border border-[#16136a]/10 bg-white shadow-2xl shadow-[#16136a]/10">
                    ...
                    <form action="{{ route('student.payments.manual.store', $due) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
                        ...
                        <div x-data="{ 
                            ...
                            <div class="relative group">
                                <div :class="preview ? 'border-indigo-500 bg-indigo-50/10' : 'border-slate-200 bg-slate-50/50 hover:bg-slate-100/50'" 
                                    ...
                                    <div class="text-center p-8 transition-all duration-300" :class="preview ? 'opacity-0 scale-95' : 'opacity-100 scale-100'">
                                        <div class="mb-6 inline-flex h-20 w-20 items-center justify-center rounded-[2rem] bg-white shadow-xl shadow-slate-200/50 text-[#16136a] group-hover:scale-110 transition-transform">
                                    ...
                                    <template x-if="preview">
                                        <div class="absolute inset-4 overflow-hidden rounded-[2rem] shadow-2xl ring-4 ring-white">
                                            ...
                                            <div class="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white">
                                                ...
                                                <p class="text-xs font-black uppercase tracking-widest">Change Image</p>
                                                <p class="mt-2 text-[10px] font-bold text-white/60 truncate px-4" x-text="filename"></p>
                                            </div>
                                        </div>
                                    </template>
                                </div>
                            ...
                            @error('payment_proof')
                                <p class="flex items-center justify-center gap-2 text-xs font-bold text-rose-500 bg-rose-50 p-3 rounded-xl">
                        ...
                        <button type="submit" class="group relative w-full overflow-hidden rounded-[2rem] bg-[#16136a] px-10 py-5 shadow-2xl shadow-[#16136a]/30 transition-all hover:bg-[#1c198a] hover:scale-[1.01] active:scale-95">
                            <div class="flex items-center justify-center gap-4">
                                <span class="text-sm font-black uppercase tracking-[0.25em] text-white">Submit Payment for Approval</span>
