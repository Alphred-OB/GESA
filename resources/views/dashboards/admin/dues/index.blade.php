    ...
    <div x-data="{ loading: true }" x-init="setTimeout(() => { loading = false }, 600)" class="mx-auto w-full max-w-6xl space-y-10 px-5 py-10 sm:px-6 lg:px-8">
        ...
        <div x-show="!loading" x-transition.opacity.duration.200ms x-cloak class="space-y-10">
            <header class="flex flex-col gap-4 rounded-3xl border border-[#16136a]/15 bg-white/85 p-6 shadow-lg shadow-[#16136a]/10 md:flex-row md:items-center md:justify-between">
                ...
                <div class="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
                    <a href="{{ route('admin.dues.export', request()->query()) }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#16136a]/20 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#16136a] shadow-sm transition hover:-translate-y-0.5 hover:bg-white/90">
                        <i class="ri-download-2-line text-base" aria-hidden="true"></i>
                        Export Excel
                    </a>
                    <a href="{{ route('admin.dues.statistics', request()->query()) }}" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#16136a]/20 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#16136a] shadow-sm transition hover:-translate-y-0.5 hover:bg-white/90">
            ...
            <section x-data="{ showStats: true }" class="space-y-6 rounded-3xl border border-[#16136a]/10 bg-white p-6 shadow-lg shadow-[#16136a]/10">
                ...
                <div x-show="showStats" x-transition.opacity.duration.150ms class="grid gap-4 md:grid-cols-4">
                    <article class="rounded-2xl border border-[#16136a]/20 bg-[#16136a] px-6 py-5 text-white shadow-lg shadow-[#16136a]/20">
                    ...
                    <article class="rounded-2xl border border-slate-200/70 bg-slate-50/80 px-6 py-5 shadow-sm shadow-[#16136a]/5">
                        ...
                        <p class="mt-3 text-3xl font-semibold text-[#16136a]">GHS {{ number_format((float) ($totals['outstanding'] ?? 0), 2) }}</p>
                    ...
                    <article class="rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-5 text-emerald-900 shadow-sm shadow-emerald-100/70">
                        <p class="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-700">Total collected</p>
                        <p class="mt-3 text-3xl font-semibold">GHS {{ number_format((float) ($totals['collected'] ?? 0), 2) }}</p>
                        <p class="mt-2 text-xs text-emerald-700/80">Sum of dues marked as paid.</p>
                    </article>
                        ...
                        @forelse ($dues as $due)
                            <article class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
                                <header class="space-y-1">
                                    <h2 class="text-base font-semibold text-slate-900">{{ $due->payment_reference ?? $due->reference_number ?? '—' }}</h2>
